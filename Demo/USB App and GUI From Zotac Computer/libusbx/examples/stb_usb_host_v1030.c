...
#define FULL_TO_SLEEP_TIMEOUT			5
#define SLEEP_TO_HIBERNATE_TIMEOUT		7
...
void set_HA_active_now(void)
{
	...
	printf("Set current time active: %d:%d\n",
		                             hour, min);
}
...
void next_power_mode(unsigned char msg)
{
	...
	case POWER_SLEEP: 
		switch (msg) {
		case BUTTON_PRESSED:
			printf("SLEEP --> FULL: button pressed!\n");
